...
function ProductForm() {
  ...
  useEffect(() => {
    async function loadMasterConfig() {
      try {
        const [catRes, typesRes, itemsRes] = await Promise.all([
        ...
        if (itemsRes.success) {
          setMasterItems(itemsRes.items || []);
        }
      } catch (err) {
        console.error('Failed to load master config in seller form:', err);
      }
    }
    loadMasterConfig();
  }, []);
  ...
  // Pre-fill master data first items if empty on create mode
  useEffect(() => {
    if (!isEditMode && masterTypes.length > 0) {
      setFormData(prev => {
        const firstVals = {};
        masterTypes.forEach(t => {
          const items = masterItems.filter(item => item.typeId === t.id && item.isActive);
          if (items.length > 0 && !prev[t.slug]) {
            firstVals[t.slug] = items[0].name;
          }
        });
        return {
          ...prev,
  ...
  }, [isEditMode, masterTypes, masterItems]);
  ...
  const handleVideoUpload = (e) => {
    ...
    const token = localStorage.getItem('hs_seller_token') || 'demo_token';
    
    // Determine API host dynamically
    const isLocal = window.location.hostname === 'localhost';
    const uploadUrl = isLocal 
      ? 'http://localhost:5001/api/sellers/upload/video'
      : `${window.location.origin}/api/sellers/upload/video`;
  ...
  const handleRemoveVideo = async () => {
    ...
    try {
      ...
      const deleteUrl = isLocal 
        ? 'http://localhost:5001/api/sellers/upload/delete-video'
        : `${window.location.origin}/api/sellers/upload/delete-video`;
